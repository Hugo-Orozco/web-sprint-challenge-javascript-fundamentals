...
// You are invoking myFunction(), it executes everything inside, then inside the function there is a nested function called nestedFunction() that is later invoked at the end of the first function. The new function nestedFunction() that was invoked console logs the variable internal, that's how you are able to access it.
...
function animalNames(array) {
  return array.map(item => `name: ${item.animal_name}, scientific: ${item.scientific_name}`);
}
...
function CuboidMaker(object) {
  ...
  CuboidMaker.prototype.surfaceArea = function () {
    return 2 * (this.length * this.width + this.length * this.height + this.width * this.height); // (this.width * this.height * 6) - (this.length * 5);
  };
}
...
function volume(length, width, height) {
  return new CuboidMaker({ length, width, height }).volume();
}

console.log(volume(4, 5, 5));
...
function surfaceArea(length, width, height) {
  return new CuboidMaker({ length, width, height }).surfaceArea();
}

console.log(surfaceArea(4, 5, 5));
...
class CubeMaker extends CuboidMakerTwo {
  constructor(object) {
    super({ length: object.length, width: object.width, height: object.height });
  }
  volume() {
    return `${this.length} * ${this.width} * ${this.height}`;
  }
...
const cubeMaker = new CubeMaker({ length: 4, width: 5, height: 5 });
